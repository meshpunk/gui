...
SX126x::SX126x(Module *mod)
    : PhysicalLayer(RADIOLIB_SX126X_FREQUENCY_STEP_SIZE,
  ...
  this->irqMap[RADIOLIB_IRQ_PREAMBLE_DETECTED] =
      RADIOLIB_SX126X_IRQ_PREAMBLE_DETECTED;
  this->irqMap[RADIOLIB_IRQ_SYNC_WORD_VALID] =
...
int16_t SX126x::begin(uint8_t cr, uint8_t syncWord, uint16_t preambleLength,
  ...
  this->bandwidth =
      RADIOLIB_SX126X_LORA_BW_500_0; // initialized to 500 kHz, since lower
  ...
  int16_t state = this->modSetup(tcxoVoltage, useRegulatorLDO,
                                 RADIOLIB_SX126X_PACKET_TYPE_LORA);
...
int16_t SX126x::beginFSK(float br, float freqDev, float rxBw,
  ...
  this->crcTypeFSK =
      RADIOLIB_SX126X_GFSK_CRC_2_BYTE_INV; // CCITT CRC configuration
  this->preambleLengthFSK = preambleLength;
  this->addrComp = RADIOLIB_SX126X_GFSK_ADDRESS_FILT_OFF;

  // set module properties and perform initial setup
  int16_t state = this->modSetup(tcxoVoltage, useRegulatorLDO,
                                 RADIOLIB_SX126X_PACKET_TYPE_GFSK);
  RADIOLIB_ASSERT(state);
...
int16_t SX126x::beginLRFHSS(uint8_t bw, uint8_t cr, bool narrowGrid,
  ...
  // set module properties and perform initial setup
  int16_t state = this->modSetup(tcxoVoltage, useRegulatorLDO,
                                 RADIOLIB_SX126X_PACKET_TYPE_LR_FHSS);
  RADIOLIB_ASSERT(state);
...
int16_t SX126x::setLrFhssConfig(uint8_t bw, uint8_t cr, uint8_t hdrCount,
                                uint16_t hopSeqId) {
  // check and cache all parameters
  RADIOLIB_CHECK_RANGE((int8_t)cr, (int8_t)RADIOLIB_SX126X_LR_FHSS_CR_5_6,
                       (int8_t)RADIOLIB_SX126X_LR_FHSS_CR_1_3,
                       RADIOLIB_ERR_INVALID_CODING_RATE);
  this->lrFhssCr = cr;
  RADIOLIB_CHECK_RANGE((int8_t)bw, (int8_t)RADIOLIB_SX126X_LR_FHSS_BW_39_06,
                       (int8_t)RADIOLIB_SX126X_LR_FHSS_BW_1574_2,
  ...
  RADIOLIB_CHECK_RANGE((int16_t)hopSeqId, (int16_t)0x000, (int16_t)0x1FF,
...
int16_t SX126x::reset(bool verify) {
  ...
  this->mod->hal->digitalWrite(this->mod->getRst(),
                               this->mod->hal->GpioLevelLow);
  this->mod->hal->delay(1);
  this->mod->hal->digitalWrite(this->mod->getRst(),
                               this->mod->hal->GpioLevelHigh);
...
int16_t SX126x::receive(uint8_t *data, size_t len) {
  ...
  if (modem == RADIOLIB_SX126X_PACKET_TYPE_LORA) {
    ...
    float symbolLength = (float)(uint32_t(1) << this->spreadingFactor) /
                         (float)this->bandwidthKhz;
    ...
    float brBps = ((float)(RADIOLIB_SX126X_CRYSTAL_FREQ) * 1000000.0 * 32.0) /
                  (float)this->bitRate;
...
int16_t SX126x::transmitDirect(uint32_t frf) {
  ...
  return (this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_SET_TX_CONTINUOUS_WAVE,
                                    data, 1));
}
...
int16_t SX126x::directMode() {
  ...
  // registers
  state = this->mod->SPIsetRegValue(RADIOLIB_SX126X_REG_TX_BITBANG_ENABLE_1,
                                    RADIOLIB_SX126X_TX_BITBANG_1_ENABLED, 6, 4);
  RADIOLIB_ASSERT(state);
  state = this->mod->SPIsetRegValue(RADIOLIB_SX126X_REG_TX_BITBANG_ENABLE_0,
                                    RADIOLIB_SX126X_TX_BITBANG_0_ENABLED, 3, 0);
  RADIOLIB_ASSERT(state);
  state = this->mod->SPIsetRegValue(RADIOLIB_SX126X_REG_DIOX_OUT_ENABLE,
                                    RADIOLIB_SX126X_DIO3_OUT_DISABLED, 3, 3);
  RADIOLIB_ASSERT(state);
  state = this->mod->SPIsetRegValue(RADIOLIB_SX126X_REG_DIOX_IN_ENABLE,
                                    RADIOLIB_SX126X_DIO3_IN_ENABLED, 3, 3);
  RADIOLIB_ASSERT(state);

  // enable TxDone interrupt
  state =
      setDioIrqParams(RADIOLIB_SX126X_IRQ_TX_DONE, RADIOLIB_SX126X_IRQ_TX_DONE);
  RADIOLIB_ASSERT(state);
...
int16_t SX126x::packetMode() {
  ...
  state = this->mod->SPIsetRegValue(RADIOLIB_SX126X_REG_DIOX_IN_ENABLE,
                                    RADIOLIB_SX126X_DIO3_IN_DISABLED, 3, 3);
  RADIOLIB_ASSERT(state);
  state = this->mod->SPIsetRegValue(RADIOLIB_SX126X_REG_DIOX_OUT_ENABLE,
                                    RADIOLIB_SX126X_DIO3_OUT_ENABLED, 3, 3);
  RADIOLIB_ASSERT(state);
  state =
      this->mod->SPIsetRegValue(RADIOLIB_SX126X_REG_TX_BITBANG_ENABLE_0,
                                RADIOLIB_SX126X_TX_BITBANG_0_DISABLED, 3, 0);
  RADIOLIB_ASSERT(state);
  state =
      this->mod->SPIsetRegValue(RADIOLIB_SX126X_REG_TX_BITBANG_ENABLE_1,
                                RADIOLIB_SX126X_TX_BITBANG_1_DISABLED, 6, 4);
  RADIOLIB_ASSERT(state);
...
int16_t SX126x::sleep() { return (SX126x::sleep(true)); }
...
int16_t SX126x::sleep(bool retainConfig) {
  ...
  uint8_t sleepMode =
      RADIOLIB_SX126X_SLEEP_START_WARM | RADIOLIB_SX126X_SLEEP_RTC_OFF;
  if (!retainConfig) {
    sleepMode =
        RADIOLIB_SX126X_SLEEP_START_COLD | RADIOLIB_SX126X_SLEEP_RTC_OFF;
  }
  int16_t state = this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_SET_SLEEP,
                                            &sleepMode, 1, false, false);
...
int16_t SX126x::standby() {
  return (SX126x::standby(this->standbyXOSC ? RADIOLIB_SX126X_STANDBY_XOSC
                                            : RADIOLIB_SX126X_STANDBY_RC));
}
...
int16_t SX126x::standby(uint8_t mode, bool wakeup) {
  ...
  if (wakeup) {
    ...
    this->mod->hal->digitalWrite(this->mod->getCs(),
                                 this->mod->hal->GpioLevelLow);
  }
...
void SX126x::setDio1Action(void (*func)(void)) {
  this->mod->hal->attachInterrupt(
      this->mod->hal->pinToInterrupt(this->mod->getIrq()), func,
      this->mod->hal->GpioInterruptRising);
}

void SX126x::clearDio1Action() {
  this->mod->hal->detachInterrupt(
      this->mod->hal->pinToInterrupt(this->mod->getIrq()));
}
...
void SX126x::clearPacketReceivedAction() { this->clearDio1Action(); }

void SX126x::setPacketSentAction(void (*func)(void)) {
  this->setDio1Action(func);
}

void SX126x::clearPacketSentAction() { this->clearDio1Action(); }

void SX126x::setChannelScanAction(void (*func)(void)) {
  this->setDio1Action(func);
}

void SX126x::clearChannelScanAction() { this->clearDio1Action(); }
...
int16_t SX126x::startTransmit(const uint8_t *data, size_t len, uint8_t addr) {
  ...
  if ((this->addrComp != RADIOLIB_SX126X_GFSK_ADDRESS_FILT_OFF) &&
      (len > RADIOLIB_SX126X_MAX_PACKET_LENGTH - 1)) {
    return (RADIOLIB_ERR_PACKET_TOO_LONG);
  }
  ...
  uint8_t modem = getPacketType();
  if (modem == RADIOLIB_SX126X_PACKET_TYPE_LORA) {
    state = setPacketParams(this->preambleLengthLoRa, this->crcTypeLoRa, len,
                            this->headerType, this->invertIQEnabled);

  } else if (modem == RADIOLIB_SX126X_PACKET_TYPE_GFSK) {
    state = setPacketParamsFSK(this->preambleLengthFSK, this->crcTypeFSK,
                               this->syncWordLength, this->addrComp,
                               this->whitening, this->packetType, len);
  ...
  // set DIO mapping
  if (modem != RADIOLIB_SX126X_PACKET_TYPE_LR_FHSS) {
    state = setDioIrqParams(RADIOLIB_SX126X_IRQ_TX_DONE |
  ...
  if (modem != RADIOLIB_SX126X_PACKET_TYPE_LR_FHSS) {
    ...
    this->lrFhssHopNum = 0;
    state =
        buildLRFHSSPacket(const_cast<uint8_t *>(data), len, frame, &frameLen,
    ...
    // activate hopping
    uint8_t hopCfg[] = {RADIOLIB_SX126X_HOPPING_ENABLED, (uint8_t)frameLen,
...
int16_t SX126x::startReceive() {
  return (this->startReceive(RADIOLIB_SX126X_RX_TIMEOUT_INF,
                             RADIOLIB_IRQ_RX_DEFAULT_FLAGS,
                             RADIOLIB_IRQ_RX_DEFAULT_MASK, 0));
}

int16_t SX126x::startReceive(uint32_t timeout, RadioLibIrqFlags_t irqFlags,
                             RadioLibIrqFlags_t irqMask, size_t len) {
  (void)len;
...
int16_t SX126x::startReceiveDutyCycle(uint32_t rxPeriod, uint32_t sleepPeriod,
  ...
  int16_t state =
      startReceiveCommon(RADIOLIB_SX126X_RX_TIMEOUT_INF, irqFlags, irqMask);
  RADIOLIB_ASSERT(state);

  uint8_t data[6] = {(uint8_t)((rxPeriodRaw >> 16) & 0xFF),
                     (uint8_t)((rxPeriodRaw >> 8) & 0xFF),
                     (uint8_t)(rxPeriodRaw & 0xFF),
                     (uint8_t)((sleepPeriodRaw >> 16) & 0xFF),
                     (uint8_t)((sleepPeriodRaw >> 8) & 0xFF),
                     (uint8_t)(sleepPeriodRaw & 0xFF)};
  return (this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_SET_RX_DUTY_CYCLE, data,
                                    6));
}

int16_t SX126x::startReceiveDutyCycleAuto(uint16_t senderPreambleLength,
                                          uint16_t minSymbols,
                                          RadioLibIrqFlags_t irqFlags,
                                          RadioLibIrqFlags_t irqMask) {
  ...
  uint32_t symbolLength = ((uint32_t)(10 * 1000) << this->spreadingFactor) /
                          (10 * this->bandwidthKhz);
  uint32_t sleepPeriod = symbolLength * sleepSymbols;
  RADIOLIB_DEBUG_BASIC_PRINTLN("Auto sleep period: %lu",
  ...
  uint32_t wakePeriod = RADIOLIB_MAX(
      (symbolLength * (senderPreambleLength + 1) - (sleepPeriod - 1000)) /
          2,                            // (A)
      symbolLength * (minSymbols + 1)); //(B)
  RADIOLIB_DEBUG_BASIC_PRINTLN("Auto wake period: %lu",
                               (long unsigned int)wakePeriod);
...
int16_t SX126x::startReceiveCommon(uint32_t timeout,
  ...
  uint8_t modem = getPacketType();
  if (modem == RADIOLIB_SX126X_PACKET_TYPE_LORA) {
    state = setPacketParams(this->preambleLengthLoRa, this->crcTypeLoRa,
                            this->implicitLen, this->headerType,
...
int16_t SX126x::startChannelScan(const ChannelScanConfig_t &config) {
  ...
  // set DIO pin mapping
  state = setDioIrqParams(getIrqMapped(config.cad.irqFlags),
                          getIrqMapped(config.cad.irqMask));
  RADIOLIB_ASSERT(state);
  ...
  // set mode to CAD
  state = setCad(config.cad.symNum, config.cad.detPeak, config.cad.detMin,
                 config.cad.exitMode, config.cad.timeout);
  return (state);
}
...
int16_t SX126x::setBandwidth(float bw) {
  ...
  return (setModulationParams(this->spreadingFactor, this->bandwidth,
                              this->codingRate, this->ldrOptimize));
}
...
int16_t SX126x::setPreambleLength(size_t preambleLength) {
  ...
  if (modem == RADIOLIB_SX126X_PACKET_TYPE_LORA) {
    this->preambleLengthLoRa = preambleLength;
    return (setPacketParams(this->preambleLengthLoRa, this->crcTypeLoRa,
                            this->implicitLen, this->headerType,
                            this->invertIQEnabled));
  } else if (modem == RADIOLIB_SX126X_PACKET_TYPE_GFSK) {
    this->preambleLengthFSK = preambleLength;
    this->preambleDetLength =
        preambleLength >= 32   ? RADIOLIB_SX126X_GFSK_PREAMBLE_DETECT_32
    ...
    return (setPacketParamsFSK(this->preambleLengthFSK, this->preambleDetLength,
                               this->crcTypeFSK, this->syncWordLength,
                               this->addrComp, this->whitening,
                               this->packetType));
  }
...
int16_t SX126x::setFrequencyDeviation(float freqDev) {
  ...
  RADIOLIB_CHECK_RANGE(newFreqDev, 0.6, 200.0,
                       RADIOLIB_ERR_INVALID_FREQUENCY_DEVIATION);

  // calculate raw frequency deviation value
  uint32_t freqDevRaw =
      (uint32_t)(((newFreqDev * 1000.0) * (float)((uint32_t)(1) << 25)) /
                 (RADIOLIB_SX126X_CRYSTAL_FREQ * 1000000.0));

  // check modulation parameters
  this->frequencyDev = freqDevRaw;

  // update modulation parameters
  return (setModulationParamsFSK(this->bitRate, this->pulseShape,
                                 this->rxBandwidth, this->frequencyDev));
}
...
int16_t SX126x::setBitRate(float br) {
  ...
  uint32_t brRaw =
      (uint32_t)((RADIOLIB_SX126X_CRYSTAL_FREQ * 1000000.0 * 32.0) /
                 (br * 1000.0));
...
int16_t SX126x::setDataRate(DataRate_t dr) {
  ...
  if (modem == RADIOLIB_SX126X_PACKET_TYPE_GFSK) {
    ...
    this->lrFhssGridNonFcc = dr.lrFhss.narrowGrid
                                 ? RADIOLIB_SX126X_LR_FHSS_GRID_STEP_NON_FCC
                                 : RADIOLIB_SX126X_LR_FHSS_GRID_STEP_FCC;
  }
...
int16_t SX126x::checkDataRate(DataRate_t dr) {
  ...
  uint8_t modem = this->getPacketType();
  if (modem == RADIOLIB_SX126X_PACKET_TYPE_GFSK) {
    RADIOLIB_CHECK_RANGE(dr.fsk.bitRate, 0.6, 300.0,
                         RADIOLIB_ERR_INVALID_BIT_RATE);
    RADIOLIB_CHECK_RANGE(dr.fsk.freqDev, 0.6, 200.0,
                         RADIOLIB_ERR_INVALID_FREQUENCY_DEVIATION);
    return (RADIOLIB_ERR_NONE);

  } else if (modem == RADIOLIB_SX126X_PACKET_TYPE_LORA) {
    RADIOLIB_CHECK_RANGE(dr.lora.spreadingFactor, 5, 12,
                         RADIOLIB_ERR_INVALID_SPREADING_FACTOR);
    RADIOLIB_CHECK_RANGE(dr.lora.bandwidth, 0.0, 510.0,
                         RADIOLIB_ERR_INVALID_BANDWIDTH);
    RADIOLIB_CHECK_RANGE(dr.lora.codingRate, 5, 8,
                         RADIOLIB_ERR_INVALID_CODING_RATE);
    return (RADIOLIB_ERR_NONE);
  }
...
int16_t SX126x::setRxBoostedGainMode(bool rxbgm, bool persist) {
  ...
  uint8_t rxGain = rxbgm ? RADIOLIB_SX126X_RX_GAIN_BOOSTED
  ...
  if (persist) {
    ...
    uint8_t data[] = {0x01,
                      (uint8_t)((RADIOLIB_SX126X_REG_RX_GAIN >> 8) & 0xFF),
...
int16_t SX126x::setSyncWord(uint8_t *syncWord, size_t len) {
  ...
  if (modem == RADIOLIB_SX126X_PACKET_TYPE_GFSK) {
    ...
    // write sync word
    int16_t state =
        writeRegister(RADIOLIB_SX126X_REG_SYNC_WORD_0, syncWord, len);
    RADIOLIB_ASSERT(state);

    // update packet parameters
    this->syncWordLength = len * 8;
    state =
        setPacketParamsFSK(this->preambleLengthFSK, this->preambleDetLength,
                           this->crcTypeFSK, this->syncWordLength,
...
int16_t SX126x::setNodeAddress(uint8_t addr) {
  ...
  int16_t state = setPacketParamsFSK(
      this->preambleLengthFSK, this->preambleDetLength, this->crcTypeFSK,
      this->syncWordLength, this->addrComp, this->whitening, this->packetType);
  RADIOLIB_ASSERT(state);
...
int16_t SX126x::setCRC(uint8_t len, uint16_t initial, uint16_t polynomial,
                       bool inverted) {
  // check active modem
  uint8_t modem = getPacketType();
  ...
  if (modem == RADIOLIB_SX126X_PACKET_TYPE_GFSK) {
    ...
    uint8_t data[2] = {(uint8_t)((initial >> 8) & 0xFF),
...
float SX126x::getDataRate() const { return (this->dataRateMeasured); }

float SX126x::getRSSI() { return (this->getRSSI(true)); }
...
float SX126x::getFrequencyError() {
  ...
  RADIOLIB_ASSERT(state);
  uint32_t efe =
      ((uint32_t)efeRaw[0] << 16) | ((uint32_t)efeRaw[1] << 8) | efeRaw[2];
  efe &= 0x0FFFFF;
...
size_t SX126x::getPacketLength(bool update, uint8_t *offset) {
  ...
  this->mod->SPIreadStream(RADIOLIB_SX126X_CMD_GET_RX_BUFFER_STATUS,
                           rxBufStatus, 2);
...
RadioLibTime_t SX126x::getTimeOnAir(size_t len) {
  ...
  uint8_t modem = getPacketType();
  if (modem == RADIOLIB_SX126X_PACKET_TYPE_LORA) {
    uint32_t symbolLength_us =
        ((uint32_t)(1000 * 10) << this->spreadingFactor) /
        (this->bandwidthKhz * 10);
    ...
    const int8_t N_symbol_header =
        this->headerType == RADIOLIB_SX126X_LORA_HEADER_EXPLICIT ? 20 : 0;

    // numerator of equation in section 6.1.4 of SX1268 datasheet v1.1 (might
    // not actually be bitcount, but it has len * 8)
    int16_t bitCount = (int16_t)8 * len + this->crcTypeLoRa * bitsPerCrc -
                       4 * this->spreadingFactor + sfCoeff2 + N_symbol_header;
    if (bitCount < 0) {
      bitCount = 0;
    }
    ...
    uint32_t nSymbol_x4 = (this->preambleLengthLoRa + 8) * 4 + sfCoeff1_x4 +
                          nPreCodedSymbols * (this->codingRate + 4) * 4;

    return ((symbolLength_us * nSymbol_x4) / 4);

  } else if (modem == RADIOLIB_SX126X_PACKET_TYPE_GFSK) {
    return (((uint32_t)len * 8 * this->bitRate) /
            (RADIOLIB_SX126X_CRYSTAL_FREQ * 32));
    ...
    case RADIOLIB_SX126X_LR_FHSS_CR_5_6:
      N_bits =
          ((len * 6) + 4) /
          5; // this is from the official LR11xx driver, but why the extra +4?
      break;
    ...
    uint16_t N_payBits = (N_bits / RADIOLIB_SX126X_LR_FHSS_FRAG_BITS) *
                         RADIOLIB_SX126X_LR_FHSS_BLOCK_BITS;
    ...
    // add header bits
    uint16_t N_totalBits =
        (RADIOLIB_SX126X_LR_FHSS_HEADER_BITS * this->lrFhssHdrCount) +
        N_payBits;
...
uint8_t SX126x::randomByte() {
  ...
  this->mod->SPIsetRegValue(RADIOLIB_SX126X_REG_ANA_LNA,
                            RADIOLIB_SX126X_LNA_RNG_ENABLED, 0, 0);
  this->mod->SPIsetRegValue(RADIOLIB_SX126X_REG_ANA_MIXER,
                            RADIOLIB_SX126X_MIXER_RNG_ENABLED, 0, 0);
  ...
  // restore the magic registers
  this->mod->SPIsetRegValue(RADIOLIB_SX126X_REG_ANA_LNA,
                            RADIOLIB_SX126X_LNA_RNG_DISABLED, 0, 0);
  this->mod->SPIsetRegValue(RADIOLIB_SX126X_REG_ANA_MIXER,
                            RADIOLIB_SX126X_MIXER_RNG_DISABLED, 0, 0);
...
void SX126x::setDirectAction(void (*func)(void)) { setDio1Action(func); }
...
int16_t SX126x::uploadPatch(const uint32_t *patch, size_t len,
                            bool nonvolatile) {
...
// check the version
  ...
  char ver_pre[16];
  this->mod->SPIreadRegisterBurst(RADIOLIB_SX126X_REG_VERSION_STRING, 16,
                                  reinterpret_cast<uint8_t *>(ver_pre));
  RADIOLIB_DEBUG_BASIC_PRINTLN("Pre-update version string: %s", ver_pre);
#endif

  // enable patch update
  this->mod->SPIwriteRegister(RADIOLIB_SX126X_REG_PATCH_UPDATE_ENABLE,
                              RADIOLIB_SX126X_PATCH_UPDATE_ENABLED);
  ...
  for (uint32_t i = 0; i < len / sizeof(uint32_t); i++) {
    ...
    data[2] = (bin >> 8) & 0xFF;
    data[3] = bin & 0xFF;
    this->mod->SPIwriteRegisterBurst(RADIOLIB_SX126X_REG_PATCH_MEMORY_BASE +
                                         i * sizeof(uint32_t),
  ...
  this->mod->SPIwriteRegister(RADIOLIB_SX126X_REG_PATCH_UPDATE_ENABLE,
                              RADIOLIB_SX126X_PATCH_UPDATE_DISABLED);
...
// check the version again
  ...
  char ver_post[16];
  this->mod->SPIreadRegisterBurst(RADIOLIB_SX126X_REG_VERSION_STRING, 16,
                                  reinterpret_cast<uint8_t *>(ver_post));
...
int16_t SX126x::spectralScanStart(uint16_t numSamples, uint8_t window,
                                  uint8_t interval) {
  ...
  uint8_t data[3] = {(uint8_t)((numSamples >> 8) & 0xFF),
                     (uint8_t)(numSamples & 0xFF), interval};
  return (this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_SET_SPECTR_SCAN_PARAMS,
                                    data, 3));
}
...
int16_t SX126x::spectralScanGetStatus() {
  uint8_t status =
      this->mod->SPIreadRegister(RADIOLIB_SX126X_REG_SPECTRAL_SCAN_STATUS);
...
int16_t SX126x::spectralScanGetResult(uint16_t *results) {
  ...
  this->mod->SPIreadRegisterBurst(RADIOLIB_SX126X_REG_SPECTRAL_SCAN_RESULT,
                                  2 * RADIOLIB_SX126X_SPECTRAL_SCAN_RES_SIZE,
                                  data);
...
int16_t SX126x::setTCXO(float voltage, uint32_t delay) {
  ...
  return (this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_SET_DIO3_AS_TCXO_CTRL,
                                    data, 4));
}
...
int16_t SX126x::setDio2AsRfSwitch(bool enable) {
  ...
  return (this->mod->SPIwriteStream(
      RADIOLIB_SX126X_CMD_SET_DIO2_AS_RF_SWITCH_CTRL, &data, 1));
}
...
int16_t SX126x::setTx(uint32_t timeout) {
  uint8_t data[] = {(uint8_t)((timeout >> 16) & 0xFF),
...
int16_t SX126x::setRx(uint32_t timeout) {
  uint8_t data[] = {(uint8_t)((timeout >> 16) & 0xFF),
                    (uint8_t)((timeout >> 8) & 0xFF),
                    (uint8_t)(timeout & 0xFF)};
  return (this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_SET_RX, data, 3, true,
                                    false));
}

int16_t SX126x::setCad(uint8_t symbolNum, uint8_t detPeak, uint8_t detMin,
                       uint8_t exitMode, RadioLibTime_t timeout) {
  ...
  int16_t state =
      this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_SET_CAD_PARAMS, data, 7);
  RADIOLIB_ASSERT(state);
...
int16_t SX126x::setPaConfig(uint8_t paDutyCycle, uint8_t deviceSel,
                            uint8_t hpMax, uint8_t paLut) {
  uint8_t data[] = {paDutyCycle, hpMax, deviceSel, paLut};
  return (
      this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_SET_PA_CONFIG, data, 4));
}
...
int16_t SX126x::setDioIrqParams(uint16_t irqMask, uint16_t dio1Mask,
  ...
  return (this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_SET_DIO_IRQ_PARAMS,
                                    data, 8));
}

int16_t SX126x::clearIrqStatus(uint16_t clearIrqParams) {
  uint8_t data[] = {(uint8_t)((clearIrqParams >> 8) & 0xFF),
                    (uint8_t)(clearIrqParams & 0xFF)};
  return (
      this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_CLEAR_IRQ_STATUS, data, 2));
}

int16_t SX126x::setRfFrequency(uint32_t frf) {
  uint8_t data[] = {(uint8_t)((frf >> 24) & 0xFF),
                    (uint8_t)((frf >> 16) & 0xFF), (uint8_t)((frf >> 8) & 0xFF),
                    (uint8_t)(frf & 0xFF)};
  return (
      this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_SET_RF_FREQUENCY, data, 4));
}
...
int16_t SX126x::calibrateImage(float freq) {
  ...
  if (data[0]) {
    ...
    RADIOLIB_DEBUG_BASIC_PRINTLN(
        "Failed to match predefined frequency range, trying custom");
...
int16_t SX126x::calibrateImageRejection(float freqMin, float freqMax) {
  ...
  uint8_t data[] = {(uint8_t)floor((freqMin - 1.0f) / 4.0f),
...
int16_t SX126x::calibrateImage(uint8_t *data) {
  int16_t state =
      this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_CALIBRATE_IMAGE, data, 2);
...
// if something failed, show the device errors
  ...
  if (state != RADIOLIB_ERR_NONE) {
    ...
    RADIOLIB_DEBUG_BASIC_PRINTLN("Calibration failed, device errors: 0x%X",
                                 errors);
  }
#endif
  return (state);
...
int16_t SX126x::setTxParams(uint8_t pwr, uint8_t rampTime) {
  ...
  int16_t state =
      this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_SET_TX_PARAMS, data, 2);
  if (state == RADIOLIB_ERR_NONE) {
    this->pwr = pwr;
  }
  return (state);
...
int16_t SX126x::setPacketMode(uint8_t mode, uint8_t len) {
  ...
  int16_t state = setPacketParamsFSK(
      this->preambleLengthFSK, this->preambleDetLength, this->crcTypeFSK,
      this->syncWordLength, this->addrComp, this->whitening, mode, len);
  RADIOLIB_ASSERT(state);
...
int16_t SX126x::setHeaderType(uint8_t hdrType, size_t len) {
  ...
  // set requested packet mode
  int16_t state = setPacketParams(this->preambleLengthLoRa, this->crcTypeLoRa,
                                  len, hdrType, this->invertIQEnabled);
  RADIOLIB_ASSERT(state);
...
int16_t SX126x::setModulationParams(uint8_t sf, uint8_t bw, uint8_t cr,
                                    uint8_t ldro) {
  ...
  return (this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_SET_MODULATION_PARAMS,
                                    data, 4));
}

int16_t SX126x::setModulationParamsFSK(uint32_t br, uint8_t sh, uint8_t rxBw,
                                       uint32_t freqDev) {
  uint8_t data[8] = {(uint8_t)((br >> 16) & 0xFF),
                     (uint8_t)((br >> 8) & 0xFF),
                     (uint8_t)(br & 0xFF),
                     sh,
                     rxBw,
...
int16_t SX126x::setPacketParams(uint16_t preambleLen, uint8_t crcType,
                                uint8_t payloadLen, uint8_t hdrType,
                                uint8_t invertIQ) {
  int16_t state = fixInvertedIQ(invertIQ);
  RADIOLIB_ASSERT(state);
  uint8_t data[6] = {(uint8_t)((preambleLen >> 8) & 0xFF),
                     (uint8_t)(preambleLen & 0xFF),
                     hdrType,
                     payloadLen,
                     crcType,
                     invertIQ};
  return (this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_SET_PACKET_PARAMS, data,
                                    6));
}

int16_t SX126x::setPacketParamsFSK(uint16_t preambleLen,
                                   uint8_t preambleDetectorLen, uint8_t crcType,
                                   uint8_t syncWordLen, uint8_t addrCmp,
                                   uint8_t whiten, uint8_t packType,
                                   uint8_t payloadLen) {
  uint8_t data[9] = {(uint8_t)((preambleLen >> 8) & 0xFF),
                     (uint8_t)(preambleLen & 0xFF),
                     preambleDetectorLen,
                     syncWordLen,
                     addrCmp,
...
int16_t SX126x::setBufferBaseAddress(uint8_t txBaseAddress,
                                     uint8_t rxBaseAddress) {
  uint8_t data[2] = {txBaseAddress, rxBaseAddress};
  return (this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_SET_BUFFER_BASE_ADDRESS,
                                    data, 2));
}

int16_t SX126x::setRegulatorMode(uint8_t mode) {
  uint8_t data[1] = {mode};
  return (this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_SET_REGULATOR_MODE,
                                    data, 1));
}
...
int16_t SX126x::clearDeviceErrors() {
  ...
  return (this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_CLEAR_DEVICE_ERRORS,
                                    data, 2));
}
...
int16_t SX126x::setFrequencyRaw(float freq) {
  ...
  this->freqMHz = freq;
  uint32_t frf =
      (this->freqMHz * (uint32_t(1) << RADIOLIB_SX126X_DIV_EXPONENT)) /
      RADIOLIB_SX126X_CRYSTAL_FREQ;
...
int16_t SX126x::fixSensitivity() {
  ...
  int16_t state = readRegister(RADIOLIB_SX126X_REG_SENSITIVITY_CONFIG,
                               &sensitivityConfig, 1);
  RADIOLIB_ASSERT(state);

  // fix the value for LoRa with 500 kHz bandwidth
  if ((getPacketType() == RADIOLIB_SX126X_PACKET_TYPE_LORA) &&
      (fabsf(this->bandwidthKhz - 500.0) <= 0.001)) {
    sensitivityConfig &= 0xFB;
  } else {
    sensitivityConfig |= 0x04;
  }
  return (writeRegister(RADIOLIB_SX126X_REG_SENSITIVITY_CONFIG,
                        &sensitivityConfig, 1));
}
...
int16_t SX126x::fixPaClamping(bool enable) {
  ...
  int16_t state =
      readRegister(RADIOLIB_SX126X_REG_TX_CLAMP_CONFIG, &clampConfig, 1);
  RADIOLIB_ASSERT(state);
...
int16_t SX126x::fixInvertedIQ(uint8_t iqConfig) {
  ...
  int16_t state =
      readRegister(RADIOLIB_SX126X_REG_IQ_CONFIG, &iqConfigCurrent, 1);
  RADIOLIB_ASSERT(state);
...
Module *SX126x::getMod() { return (this->mod); }

int16_t SX126x::modSetup(float tcxoVoltage, bool useRegulatorLDO,
                         uint8_t modem) {
  // set module properties
  this->mod->init();

  RADIOLIB_DEBUG_BASIC_PRINTLN("Setting up SX126x");

  this->mod->hal->pinMode(this->mod->getIrq(), this->mod->hal->GpioModeInput);
  RADIOLIB_DEBUG_BASIC_PRINTLN("IRQ pin mode set to %d", this->mod->getIrq());

  this->mod->hal->pinMode(this->mod->getGpio(), this->mod->hal->GpioModeInput);
  RADIOLIB_DEBUG_BASIC_PRINTLN("GPIO pin mode set to %d", this->mod->getGpio());
  ...
  this->mod->spiConfig.cmds[RADIOLIB_MODULE_SPI_COMMAND_READ] =
      RADIOLIB_SX126X_CMD_READ_REGISTER;
  this->mod->spiConfig.cmds[RADIOLIB_MODULE_SPI_COMMAND_WRITE] =
      RADIOLIB_SX126X_CMD_WRITE_REGISTER;
  this->mod->spiConfig.cmds[RADIOLIB_MODULE_SPI_COMMAND_NOP] =
      RADIOLIB_SX126X_CMD_NOP;
  this->mod->spiConfig.cmds[RADIOLIB_MODULE_SPI_COMMAND_STATUS] =
...
int16_t SX126x::config(uint8_t modem) {
  ...
  data[0] = modem;
  state =
      this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_SET_PACKET_TYPE, data, 1);
  RADIOLIB_ASSERT(state);
  ...
  state = this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_SET_RX_TX_FALLBACK_MODE,
                                    data, 1);
  RADIOLIB_ASSERT(state);
  ...
  state = this->mod->SPIwriteStream(RADIOLIB_SX126X_CMD_CALIBRATE, data, 1,
                                    true, false);
  RADIOLIB_ASSERT(state);
...
bool SX126x::findChip(const char *verStr) {
  ...
  while ((i < 10) && !flagFound) {
    ...
    if (strncmp(verStr, version, 6) == 0) {
      ...
      RADIOLIB_DEBUG_BASIC_HEXDUMP(reinterpret_cast<uint8_t *>(version), 16,

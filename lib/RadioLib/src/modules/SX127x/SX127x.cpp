...
SX127x::SX127x(Module *mod)
    : PhysicalLayer(RADIOLIB_SX127X_FREQUENCY_STEP_SIZE,
                    RADIOLIB_SX127X_MAX_PACKET_LENGTH) {
  this->mod = mod;
}

int16_t SX127x::begin(uint8_t *chipVersions, uint8_t numVersions,
  ...
  this->irqMap[RADIOLIB_IRQ_CAD_DETECTED] =
      RADIOLIB_SX127X_CLEAR_IRQ_FLAG_CAD_DETECTED;
  this->irqMap[RADIOLIB_IRQ_TIMEOUT] =
...
int16_t SX127x::beginFSK(uint8_t *chipVersions, uint8_t numVersions,
                         float freqDev, float rxBw, uint16_t preambleLength,
                         bool enableOOK) {
  // set module properties
  this->mod->init();
  this->mod->hal->pinMode(this->mod->getIrq(), this->mod->hal->GpioModeInput);
  ...
  this->irqMap[RADIOLIB_IRQ_PREAMBLE_DETECTED] =
      RADIOLIB_SX127X_FLAG_PREAMBLE_DETECT << 0;
  this->irqMap[RADIOLIB_IRQ_SYNC_WORD_VALID] =
      RADIOLIB_SX127X_FLAG_SYNC_ADDRESS_MATCH << 0;
...
int16_t SX127x::receive(uint8_t *data, size_t len) {
  ...
  if (modem == RADIOLIB_SX127X_LORA) {
    // set mode to receive
    state = startReceive(100, RADIOLIB_IRQ_RX_DEFAULT_FLAGS,
                         RADIOLIB_IRQ_RX_DEFAULT_MASK, len);
    RADIOLIB_ASSERT(state);

    // if no DIO1 is provided, use software timeout (100 LoRa symbols, same as
    // hardware timeout)
    RadioLibTime_t timeout = 0;
    ...
    // set mode to receive
    state = startReceive(0, RADIOLIB_IRQ_RX_DEFAULT_FLAGS,
                         RADIOLIB_IRQ_RX_DEFAULT_MASK, len);
    RADIOLIB_ASSERT(state);
...
int16_t SX127x::transmitDirect(uint32_t frf) {
  ...
  if (frf != 0) {
    this->mod->SPIwriteRegister(RADIOLIB_SX127X_REG_FRF_MSB,
                                (frf & 0xFF0000) >> 16);
    this->mod->SPIwriteRegister(RADIOLIB_SX127X_REG_FRF_MID,
                                (frf & 0x00FF00) >> 8);
...
int16_t SX127x::directMode() {
  ...
  // set continuous mode
  return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_2,
                                    RADIOLIB_SX127X_DATA_MODE_CONTINUOUS, 6,
                                    6));
}
...
int16_t SX127x::packetMode() {
  ...
  return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_2,
                                    RADIOLIB_SX127X_DATA_MODE_PACKET, 6, 6));
}

int16_t SX127x::startReceive() {
  return (this->startReceive(0, RADIOLIB_IRQ_RX_DEFAULT_FLAGS,
                             RADIOLIB_IRQ_RX_DEFAULT_MASK, 0));
}

int16_t SX127x::startReceive(uint32_t timeout, RadioLibIrqFlags_t irqFlags,
  ...
  if (modem == RADIOLIB_SX127X_LORA) {
    if (timeout != 0) {
      ...
      state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_MODEM_CONFIG_2,
                                        msb_sym, 1, 0);
      state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_SYMB_TIMEOUT_LSB,
                                         lsb_sym);
      RADIOLIB_ASSERT(state);
    }

    // in FHSS mode, enable channel change interrupt
    if (this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_HOP_PERIOD) >
        RADIOLIB_SX127X_HOP_PERIOD_OFF) {
      state = this->mod->SPIsetRegValue(
          RADIOLIB_SX127X_REG_DIO_MAPPING_1,
    ...
    if (this->spreadingFactor == 6) {
      state |=
          this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PAYLOAD_LENGTH, len);
...
void SX127x::setDio0Action(void (*func)(void), uint32_t dir) {
  this->mod->hal->attachInterrupt(
      this->mod->hal->pinToInterrupt(this->mod->getIrq()), func, dir);
}
...
void SX127x::setDio1Action(void (*func)(void), uint32_t dir) {
  ...
  this->mod->hal->attachInterrupt(
      this->mod->hal->pinToInterrupt(this->mod->getGpio()), func, dir);
}
...
void SX127x::clearDio1Action() {
  ...
  this->mod->hal->detachInterrupt(
      this->mod->hal->pinToInterrupt(this->mod->getGpio()));
}
...
void SX127x::clearPacketReceivedAction() { this->clearDio0Action(); }

void SX127x::setPacketSentAction(void (*func)(void)) {
  this->setDio0Action(func, this->mod->hal->GpioInterruptRising);
}

void SX127x::clearPacketSentAction() { this->clearDio0Action(); }

void SX127x::setChannelScanAction(void (*func)(void)) {
  this->setDio0Action(func, this->mod->hal->GpioInterruptRising);
}

void SX127x::clearChannelScanAction() { this->clearDio0Action(); }
...
void SX127x::setFifoFullAction(void (*func)(void)) {
  ...
  this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_FIFO_THRESH,
                            RADIOLIB_SX127X_FIFO_THRESH, 5, 0);
  this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_1,
                            RADIOLIB_SX127X_DIO1_PACK_FIFO_LEVEL, 5, 4);
...
bool SX127x::fifoAdd(uint8_t *data, int totalLen, int *remLen) {
  ...
  *remLen -= RADIOLIB_SX127X_FIFO_THRESH - 1;
  ...
  this->mod->SPIwriteRegisterBurst(RADIOLIB_SX127X_REG_FIFO,
                                   &data[totalLen - *remLen], len);

  // we're not done yet
  return (false);
}

bool SX127x::fifoGet(volatile uint8_t *data, int totalLen,
                     volatile int *rcvLen) {
...
int16_t SX127x::startTransmit(const uint8_t *data, size_t len, uint8_t addr) {
  ...
  if (modem == RADIOLIB_SX127X_LORA) {
    ...
    if (this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_HOP_PERIOD) >
      ...
      this->mod->SPIsetRegValue(
          RADIOLIB_SX127X_REG_DIO_MAPPING_1,
          RADIOLIB_SX127X_DIO0_LORA_TX_DONE |
              RADIOLIB_SX127X_DIO1_LORA_FHSS_CHANGE_CHANNEL,
          7, 4);
    } else {
      this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_1,
                                RADIOLIB_SX127X_DIO0_LORA_TX_DONE, 7, 6);
    }
    ...
    if (len > RADIOLIB_SX127X_MAX_PACKET_LENGTH_FSK) {
      this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_1,
                                RADIOLIB_SX127X_DIO1_PACK_FIFO_EMPTY, 5, 4);
    } else {
      this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_1,
                                RADIOLIB_SX127X_DIO0_PACK_PACKET_SENT, 7, 6);
    }

    // set packet length - increased by 1 when address filter is enabled
    uint8_t filter =
        this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1, 2, 1);
  ...
  if ((modem == RADIOLIB_SX127X_FSK_OOK) &&
      (len > RADIOLIB_SX127X_MAX_PACKET_LENGTH_FSK)) {
    packetLen = RADIOLIB_SX127X_FIFO_THRESH - 1;
    this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_FIFO_THRESH,
                              RADIOLIB_SX127X_TX_START_FIFO_NOT_EMPTY, 7, 7);
  }
  this->mod->SPIwriteRegisterBurst(RADIOLIB_SX127X_REG_FIFO,
                                   const_cast<uint8_t *>(data), packetLen);
...
int16_t SX127x::readData(uint8_t *data, size_t len) {
  ...
  if (modem == RADIOLIB_SX127X_LORA) {
    ...
    if (this->crcEnabled && (state == RADIOLIB_ERR_NONE) &&
        (this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_HOP_CHANNEL, 6, 6) ==
         0)) {
    ...
    int16_t addr =
        this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_FIFO_RX_CURRENT_ADDR);
    if (addr >= 0) {
...
int16_t SX127x::startChannelScan() {
  ...
  state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_1,
                                    RADIOLIB_SX127X_DIO0_LORA_CAD_DONE |
                                        RADIOLIB_SX127X_DIO1_LORA_CAD_DETECTED,
                                    7, 4);
  RADIOLIB_ASSERT(state);
...
int16_t SX127x::setCurrentLimit(uint8_t currentLimit) {
  // check allowed range
  if (!(((currentLimit >= 45) && (currentLimit <= 240)) ||
        (currentLimit == 0))) {
  ...
  if (currentLimit == 0) {
    ...
    state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_OCP,
                                       RADIOLIB_SX127X_OCP_OFF, 5, 5);
  } else if (currentLimit <= 120) {
    raw = (currentLimit - 45) / 5;
    state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_OCP,
                                       RADIOLIB_SX127X_OCP_ON | raw, 5, 0);
...
int16_t SX127x::setPreambleLength(size_t preambleLength) {
  ...
  if (modem == RADIOLIB_SX127X_LORA) {
    ...
    // set preamble length
    state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PREAMBLE_MSB,
                                      (uint8_t)((preambleLength >> 8) & 0xFF));
    state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PREAMBLE_LSB,
    ...
    state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PREAMBLE_MSB_FSK,
                                      (uint8_t)((numBytes >> 8) & 0xFF));
    state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PREAMBLE_LSB_FSK,
                                       (uint8_t)(numBytes & 0xFF));
    return (state);
  }
...
int16_t SX127x::invertPreamble(bool enable) {
  ...
  if (modem == RADIOLIB_SX127X_LORA) {
    ...
    // set preamble polarity
    uint8_t polarity = enable ? RADIOLIB_SX127X_PREAMBLE_POLARITY_AA
                              : RADIOLIB_SX127X_PREAMBLE_POLARITY_55;
    state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_SYNC_CONFIG, polarity,
                                      5, 5);
    return (state);
  }
...
float SX127x::getFrequencyError(bool autoCorrect) {
  ...
  if (modem == RADIOLIB_SX127X_LORA) {
    ...
    uint32_t raw =
        (uint32_t)this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_FEI_MSB, 3, 0)
        << 16;
    raw |= (uint16_t)this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_FEI_MID)
           << 8;
    ...
    if (raw & 0x80000) {
      ...
      raw = ~raw + 1;
      error = (((float)raw * (float)base) / 32000000.0) *
              (this->bandwidth / 500.0) * -1.0;
    } else {
      error =
          (((float)raw * (float)base) / 32000000.0) * (this->bandwidth / 500.0);
    }
    ...
    // get raw frequency error
    uint16_t raw =
        (uint16_t)this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_FEI_MSB_FSK)
        << 8;
...
float SX127x::getAFCError() {
  ...
  // get raw frequency error
  int16_t raw =
      (uint16_t)this->mod->SPIreadRegister(RADIOLIB_SX127X_REG_AFC_MSB) << 8;
...
float SX127x::getSNR() {
  ...
  // get SNR value
  int8_t rawSNR =
      (int8_t)this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_PKT_SNR_VALUE);
  return (rawSNR / 4.0);
}

float SX127x::getDataRate() const { return (this->dataRate); }
...
int16_t SX127x::setBitRateCommon(float br, uint8_t fracRegAddr) {
  ...
  state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_BITRATE_MSB,
                                    (bitRateRaw & 0xFF00) >> 8, 7, 0);
  state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_BITRATE_LSB,
                                     bitRateRaw & 0x00FF, 7, 0);

  // set fractional part of bit rate
  if (!ookEnabled) {
    float bitRateRem = ((RADIOLIB_SX127X_CRYSTAL_FREQ * 1000.0) / (float)br) -
                       (float)bitRateRaw;
...
int16_t SX127x::setFrequencyDeviation(float freqDev) {
  ...
  state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_FDEV_MSB,
                                    (FDEV & 0xFF00) >> 8, 5, 0);
  state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_FDEV_LSB,
                                     FDEV & 0x00FF, 7, 0);
  return (state);
}
...
uint8_t SX127x::calculateBWManExp(float bandwidth) {
  for (uint8_t e = 7; e >= 1; e--) {
    for (int8_t m = 2; m >= 0; m--) {
      float point = (RADIOLIB_SX127X_CRYSTAL_FREQ * 1000000.0) /
...
int16_t SX127x::setRxBandwidth(float rxBw) {
  ...
  // set Rx bandwidth
  return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_RX_BW,
                                    calculateBWManExp(rxBw), 4, 0));
}
...
int16_t SX127x::setAFCBandwidth(float rxBw) {
  ...
  // set AFC bandwidth
  return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_AFC_BW,
                                    calculateBWManExp(rxBw), 4, 0));
}
...
int16_t SX127x::setAFC(bool isEnabled) {
  ...
  return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_RX_CONFIG,
                                    isEnabled ? RADIOLIB_SX127X_AFC_AUTO_ON
                                              : RADIOLIB_SX127X_AFC_AUTO_OFF,
                                    4, 4));
}
...
int16_t SX127x::setAFCAGCTrigger(uint8_t trigger) {
  ...
  // set AFC&AGC trigger
  return (
      this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_RX_CONFIG, trigger, 2, 0));
}
...
int16_t SX127x::setSyncWord(uint8_t *syncWord, size_t len) {
  ...
  if (modem == RADIOLIB_SX127X_FSK_OOK) {
    ...
    if (len == 0) {
      int16_t state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_SYNC_CONFIG,
                                                RADIOLIB_SX127X_SYNC_OFF, 4, 4);
      return (state);
    }
    ...
    int16_t state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_SYNC_CONFIG,
                                              RADIOLIB_SX127X_SYNC_ON, 4, 4);
    state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_SYNC_CONFIG, len - 1,
                                       2, 0);
    RADIOLIB_ASSERT(state);

    // set sync word
    this->mod->SPIwriteRegisterBurst(RADIOLIB_SX127X_REG_SYNC_VALUE_1, syncWord,
                                     len);
    return (RADIOLIB_ERR_NONE);
...
int16_t SX127x::setBroadcastAddress(uint8_t broadAddr) {
  ...
  int16_t state = this->mod->SPIsetRegValue(
  ...
  return (
      this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_BROADCAST_ADRS, broadAddr));
}
...
int16_t SX127x::setOokThresholdType(uint8_t type) {
  ...
  return (
      this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_OOK_PEAK, type, 4, 3, 5));
}
...
int16_t SX127x::setOokFixedOrFloorThreshold(uint8_t value) {
  ...
  return (
      this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_OOK_FIX, value, 7, 0, 5));
}
...
int16_t SX127x::setOokPeakThresholdDecrement(uint8_t value) {
  ...
  return (
      this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_OOK_AVG, value, 7, 5, 5));
}
...
int16_t SX127x::setOokPeakThresholdStep(uint8_t value) {
  ...
  return (
      this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_OOK_PEAK, value, 2, 0, 5));
}

int16_t SX127x::enableBitSync() {
  return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_OOK_PEAK,
                                    RADIOLIB_SX127X_BIT_SYNC_ON, 5, 5, 5));
}

int16_t SX127x::disableBitSync() {
  return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_OOK_PEAK,
                                    RADIOLIB_SX127X_BIT_SYNC_OFF, 5, 5, 5));
}
...
int16_t SX127x::setOOK(bool enableOOK) {
  ...
  int16_t state = RADIOLIB_ERR_NONE;
  if (enableOOK) {
    state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_OP_MODE,
                                      RADIOLIB_SX127X_MODULATION_OOK, 6, 5, 5);
    state |=
        SX127x::setAFCAGCTrigger(RADIOLIB_SX127X_RX_TRIGGER_RSSI_INTERRUPT);
  } else {
    state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_OP_MODE,
                                      RADIOLIB_SX127X_MODULATION_FSK, 6, 5, 5);
...
int16_t SX127x::setFrequencyRaw(float newFreq) {
  ...
  uint32_t FRF = (newFreq * (uint32_t(1) << RADIOLIB_SX127X_DIV_EXPONENT)) /
                 RADIOLIB_SX127X_CRYSTAL_FREQ;

  // write registers
  state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_FRF_MSB,
                                     (FRF & 0xFF0000) >> 16);
  state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_FRF_MID,
                                     (FRF & 0x00FF00) >> 8);
  state |=
      this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_FRF_LSB, FRF & 0x0000FF);
  return (state);
}
  ...
  if (modem == RADIOLIB_SX127X_LORA) {
    ...
    if (!this->packetLengthQueried && update) {
      if (this->packetLengthConfig == RADIOLIB_SX127X_PACKET_VARIABLE) {
        ...
        this->packetLength =
            this->mod->SPIreadRegister(RADIOLIB_SX127X_REG_PAYLOAD_LENGTH_FSK);
      }
...
float SX127x::getNumSymbols(size_t len) {
  ...
  float symbolLength =
      (float)(uint32_t(1) << this->spreadingFactor) / (float)this->bandwidth;
  ...
  float ih = (float)this->mod->SPIgetRegValue(
      RADIOLIB_SX127X_REG_MODEM_CONFIG_1, 0, 0);

  // get CRC enabled flag
  float crc = (float)(this->mod->SPIgetRegValue(
                          RADIOLIB_SX127X_REG_MODEM_CONFIG_2, 2, 2) >>
                      2);

  // get number of preamble symbols
  float n_pre =
      (float)((this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_PREAMBLE_MSB)
               << 8) |
  ...
  float n_pay =
      8.0 + RADIOLIB_MAX(
                ceilf((8.0 * (float)len - 4.0 * (float)this->spreadingFactor +
                       28.0 + 16.0 * crc - 20.0 * ih) /
                      (4.0 * (float)this->spreadingFactor - 8.0 * de)) *
                    (float)this->codingRate,
                0.0);
...
RadioLibTime_t SX127x::getTimeOnAir(size_t len) {
  ...
  if (modem == RADIOLIB_SX127X_LORA) {
    ...
    float n_pre =
        (float)((this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_PREAMBLE_MSB_FSK)
                 << 8) |
                this->mod->SPIgetRegValue(
                    RADIOLIB_SX127X_REG_PREAMBLE_LSB_FSK)) *
        8;
    // get the number of bits of the sync word
    float n_syncWord = (float)(this->mod->SPIgetRegValue(
                                   RADIOLIB_SX127X_REG_SYNC_CONFIG, 2, 0) +
                               1) *
                       8;
    // get CRC bits
    float crc = (this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1,
                                           4, 4) == RADIOLIB_SX127X_CRC_ON) *
                16;
    ...
    return ((uint32_t)(((crc + n_syncWord + n_pre + (float)(len * 8)) /
                        (this->bitRate * 1000.0)) *
                       1000000.0));
  }
...
uint32_t SX127x::getIrqFlags() { return ((uint32_t)this->getIRQFlags()); }
...
int16_t SX127x::setIrqFlags(uint32_t irq) {
  ...
  for (uint8_t i = 0; i <= 31; i++) {
    ...
    if (usedPinFlags & (1UL << dioNum)) {
      ...
      RADIOLIB_DEBUG_PRINTLN("Unable to set IRQ %04x on DIO%d due to conflict!",
                             irqBit, (int)dioNum);
      conflict = true;
      continue;
    }
    ...
    uint8_t addr = (dioNum > 3) ? RADIOLIB_SX127X_REG_DIO_MAPPING_2
                                : RADIOLIB_SX127X_REG_DIO_MAPPING_1;
...
int16_t SX127x::clearIrqFlags(uint32_t irq) {
  ...
  if (modem == RADIOLIB_SX127X_LORA) {
    ...
    this->mod->SPIwriteRegister(RADIOLIB_SX127X_REG_IRQ_FLAGS_2,
                                (uint8_t)(irq >> 8));
    return (RADIOLIB_ERR_NONE);
  }
...
int16_t SX127x::setCrcFiltering(bool enable) {
  ...
  if (enable == true) {
    return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1,
                                      RADIOLIB_SX127X_CRC_ON, 4, 4));
  } else {
    return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1,
                                      RADIOLIB_SX127X_CRC_OFF, 4, 4));
  }
}
...
int16_t SX127x::setRSSIThreshold(float dbm) {
  ...
  return this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_RSSI_THRESH,
                                   (uint8_t)(-2.0 * dbm), 7, 0);
}
...
int16_t SX127x::setRSSIConfig(uint8_t smoothingSamples, int8_t offset) {
  ...
  state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_RSSI_CONFIG,
                                    offsetRaw << 3, 7, 3);
  state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_RSSI_CONFIG,
                                     smoothingSamples, 2, 0);
  return (state);
}
...
int16_t SX127x::setEncoding(uint8_t encoding) {
  ...
  case RADIOLIB_ENCODING_NRZ:
    return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1,
                                      RADIOLIB_SX127X_DC_FREE_NONE, 6, 5));
  case RADIOLIB_ENCODING_MANCHESTER:
    return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1,
                                      RADIOLIB_SX127X_DC_FREE_MANCHESTER, 6,
                                      5));
  case RADIOLIB_ENCODING_WHITENING:
    return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1,
                                      RADIOLIB_SX127X_DC_FREE_WHITENING, 6, 5));
  default:
...
uint16_t SX127x::getIRQFlags() {
  ...
  if (getActiveModem() == RADIOLIB_SX127X_LORA) {
    ...
    uint16_t flags =
        ((uint16_t)this->mod->SPIreadRegister(RADIOLIB_SX127X_REG_IRQ_FLAGS_2))
        << 8;
    flags |=
        (uint16_t)this->mod->SPIreadRegister(RADIOLIB_SX127X_REG_IRQ_FLAGS_1);
    return (flags);
  }
}
...
int8_t SX127x::getTempRaw() {
  ...
  if ((previousOpMode & RADIOLIB_SX127X_LORA) == RADIOLIB_SX127X_LORA) {
    this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_OP_MODE,
                              (RADIOLIB_SX127X_LORA | RADIOLIB_SX127X_SLEEP));
  }

  // put device in FSK sleep
  this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_OP_MODE,
                            (RADIOLIB_SX127X_FSK_OOK | RADIOLIB_SX127X_SLEEP));

  // put device in FSK RxSynth
  this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_OP_MODE,
                            (RADIOLIB_SX127X_FSK_OOK | RADIOLIB_SX127X_FSRX));

  // enable temperature reading
  this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_IMAGE_CAL,
                            RADIOLIB_SX127X_TEMP_MONITOR_ON, 0, 0);
  ...
  this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_IMAGE_CAL,
                            RADIOLIB_SX127X_TEMP_MONITOR_OFF, 0, 0);
...
Module *SX127x::getMod() { return (this->mod); }

int16_t SX127x::config() {
  // turn off frequency hopping
  int16_t state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_HOP_PERIOD,
                                            RADIOLIB_SX127X_HOP_PERIOD_OFF);
  return (state);
}
...
int16_t SX127x::configFSK() {
  // set RSSI threshold
  int16_t state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_RSSI_THRESH,
                                            RADIOLIB_SX127X_RSSI_THRESHOLD);
  RADIOLIB_ASSERT(state);

  // reset FIFO flag
  this->mod->SPIwriteRegister(RADIOLIB_SX127X_REG_IRQ_FLAGS_2,
                              RADIOLIB_SX127X_FLAG_FIFO_OVERRUN);
  ...
  state = this->mod->SPIsetRegValue(
      ...
      RADIOLIB_SX127X_PACKET_VARIABLE | RADIOLIB_SX127X_DC_FREE_NONE |
          ...
          RADIOLIB_SX127X_ADDRESS_FILTERING_OFF |
          RADIOLIB_SX127X_CRC_WHITENING_TYPE_CCITT,
      7, 0);
  state |= this->mod->SPIsetRegValue(
  ...
  state =
      this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_FIFO_THRESH,
                                RADIOLIB_SX127X_TX_START_FIFO_NOT_EMPTY, 7, 7);
  state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_FIFO_THRESH,
                                     RADIOLIB_SX127X_FIFO_THRESH, 5, 0);
  RADIOLIB_ASSERT(state);

  // disable Rx timeouts
  state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_RX_TIMEOUT_1,
                                    RADIOLIB_SX127X_TIMEOUT_RX_RSSI_OFF);
  state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_RX_TIMEOUT_2,
                                     RADIOLIB_SX127X_TIMEOUT_RX_PREAMBLE_OFF);
  state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_RX_TIMEOUT_3,
                                     RADIOLIB_SX127X_TIMEOUT_SIGNAL_SYNC_OFF);
  RADIOLIB_ASSERT(state);

  // enable preamble detector
  state =
      this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PREAMBLE_DETECT,
                                RADIOLIB_SX127X_PREAMBLE_DETECTOR_ON |
                                    RADIOLIB_SX127X_PREAMBLE_DETECTOR_2_BYTE |
...
int16_t SX127x::setPacketMode(uint8_t mode, uint8_t len) {
  ...
  state =
      this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PAYLOAD_LENGTH_FSK, len);
  RADIOLIB_ASSERT(state);
...
bool SX127x::findChip(const uint8_t *vers, uint8_t num) {
  ...
  while ((i < 10) && !flagFound) {
    ...
    if (!flagFound) {
      RADIOLIB_DEBUG_BASIC_PRINTLN("Sad SX127x not found! (%d of 10 tries) "
...
int16_t SX127x::setMode(uint8_t mode) {
  ...
  return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_OP_MODE, mode, 2, 0, 5,
                                    checkMask));
}
...
int16_t SX127x::setActiveModem(uint8_t modem) {
  ...
  // set modem
  state |=
      this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_OP_MODE, modem, 7, 7, 5);
...
int16_t SX127x::invertIQ(bool enable) {
  ...
  if (enable) {
    state =
        this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_INVERT_IQ,
                                  RADIOLIB_SX127X_INVERT_IQ_RXPATH_ON, 6, 6);
    state |=
        this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_INVERT_IQ,
                                  RADIOLIB_SX127X_INVERT_IQ_TXPATH_OFF, 0, 0);
    state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_INVERT_IQ2,
                                       RADIOLIB_SX127X_IQ2_ENABLE);
  } else {
    state =
        this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_INVERT_IQ,
                                  RADIOLIB_SX127X_INVERT_IQ_RXPATH_OFF, 6, 6);
    state |=
        this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_INVERT_IQ,
                                  RADIOLIB_SX127X_INVERT_IQ_TXPATH_ON, 0, 0);
    state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_INVERT_IQ2,
                                       RADIOLIB_SX127X_IQ2_DISABLE);
  }
...
int16_t SX127x::setFHSSHoppingPeriod(uint8_t freqHoppingPeriod) {
  return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_HOP_PERIOD,
                                    freqHoppingPeriod));
}
...
void SX127x::clearFHSSInt(void) {
  ...
  if (modem == RADIOLIB_SX127X_LORA) {
    this->mod->SPIwriteRegister(
        ...
        RADIOLIB_SX127X_CLEAR_IRQ_FLAG_FHSS_CHANGE_CHANNEL);
  } else if (modem == RADIOLIB_SX127X_FSK_OOK) {
...
int16_t SX127x::setDIOMapping(uint32_t pin, uint32_t value) {
  ...
  if (pin < 4)
    return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_1, value,
                                      7 - 2 * pin, 6 - 2 * pin));
  else
    return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_2, value,
                                      15 - 2 * pin, 14 - 2 * pin));
}
...
int16_t SX127x::setDIOPreambleDetect(bool usePreambleDetect) {
  return this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_2,
                                   (usePreambleDetect)
                                       ? RADIOLIB_SX127X_DIO_MAP_PREAMBLE_DETECT
                                       : RADIOLIB_SX127X_DIO_MAP_RSSI,
                                   0, 0);
}
...
float SX127x::getRSSI(bool packet, bool skipReceive, int16_t offset) {
  if (getActiveModem() == RADIOLIB_SX127X_LORA) {
    if (packet) {
      ...
      float lastPacketRSSI = offset + this->mod->SPIgetRegValue(
                                          RADIOLIB_SX127X_REG_PKT_RSSI_VALUE);
      ...
      float currentRSSI =
          offset + this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_RSSI_VALUE);
      return (currentRSSI);
    }
    ...
    float rssi =
        (float)this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_RSSI_VALUE_FSK) /
        -2.0;
...
int16_t SX127x::setLowBatteryThreshold(int8_t level, uint32_t pin) {
  // check disable
  if (level < 0) {
    return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_LOW_BAT,
                                      RADIOLIB_SX127X_LOW_BAT_OFF, 3, 3));
  }

  // enable detector and set the threshold
  int16_t state = this->mod->SPIsetRegValue(
      RADIOLIB_SX127X_REG_LOW_BAT, RADIOLIB_SX127X_LOW_BAT_ON | level, 3, 0);
  RADIOLIB_ASSERT(state);

...
class MyMesh : public BaseChatMesh {
    ...
    if (_fs->exists("/contacts3")) {
      ...
      if (file) {
        ...
        while (!full) {
          ...
          success = success && (file.read((uint8_t *)&c.sync_since, 4) ==
                                4); // was 'reserved'
          success = success && (file.read((uint8_t *)&c.out_path_len, 1) == 1);
          success = success &&
                    (file.read((uint8_t *)&c.last_advert_timestamp, 4) == 4);
  ...
  void saveContacts() {
    ...
    if (file) {
      ...
      while (iter.hasNext(this, c)) {
        ...
        success = success &&
                  (file.write((uint8_t *)&c.last_advert_timestamp, 4) == 4);
  ...
  int getBlobByKey(const uint8_t key[], int key_len,
    ...
    if (_fs->exists(path)) {
      ...
      if (f) {
        int len = f.read(dest_buf,
                         255); // currently MAX 255 byte blob len supported!!
        f.close();
        return len;
      }
    }
...
protected:
  ...
  void onContactPathUpdated(const ContactInfo &contact) override {
    ...
    memcpy(&out_frame[1], contact.id.pub_key, PUB_KEY_SIZE);
    _serial->writeFrame(out_frame,
                        1 + PUB_KEY_SIZE); // NOTE: app may not be connected
  ...
  bool processAck(const uint8_t *data) override {
    ...
    if (memcmp(data, &expected_ack_crc, 4) == 0) { // got an ACK from recipient
      ...
      expected_ack_crc =
          0; // reset our expected hash, now that we have received ACK
      return true;
    }
    return checkConnectionsAck(data);
  }

  void queueMessage(const ContactInfo &from, uint8_t txt_type, uint8_t path_len,
  ...
  void onMessageRecv(const ContactInfo &from, uint8_t path_len,
                     uint32_t sender_timestamp, const char *text) override {
    markConnectionActive(
        from); // in case this is from a server, and we have a connection
    queueMessage(from, TXT_TYPE_PLAIN, path_len, sender_timestamp, NULL, 0,
                 text);
  }

  void onCommandDataRecv(const ContactInfo &from, uint8_t path_len,
                         uint32_t sender_timestamp, const char *text) override {
    markConnectionActive(
        from); // in case this is from a server, and we have a connection
    queueMessage(from, TXT_TYPE_CLI_DATA, path_len, sender_timestamp, NULL, 0,
                 text);
  }

  void onSignedMessageRecv(const ContactInfo &from, uint8_t path_len,
                           uint32_t sender_timestamp,
                           const uint8_t *sender_prefix,
                           const char *text) override {
    markConnectionActive(from);
    saveContacts(); // from.sync_since change needs to be persisted
    queueMessage(from, TXT_TYPE_SIGNED_PLAIN, path_len, sender_timestamp,
                 sender_prefix, 4, text);
  }

  void onChannelMessageRecv(const mesh::GroupChannel &channel, int in_path_len,
...
#ifdef DISPLAY_CLASS
    ui_task.showMsgPreview(in_path_len < 0 ? 0xFF : in_path_len, "Public",
                           text);
#endif
  }
  ...
  void onContactResponse(const ContactInfo &contact, const uint8_t *data,
    ...
    if (pending_login && memcmp(&pending_login, contact.id.pub_key, 4) ==
                             0) { // check for login response
      ...
      _serial->writeFrame(out_frame, i);
               ...
               memcmp(&pending_status, contact.id.pub_key, 4) ==
                   0) { // check for status response
...
public:
  MyMesh(RADIO_CLASS &phy, RadioLibWrapper &rw, mesh::RNG &rng,
         mesh::RTCClock &rtc, SimpleMeshTables &tables)
      : BaseChatMesh(rw, *new ArduinoMillis(), rng, rtc,
                     *new StaticPoolPacketManager(16), tables),
        _serial(NULL), _phy(&phy) {
    _iter_started = false;
    offline_queue_len = 0;
    app_target_ver = 0;
  ...
  void begin(FILESYSTEM &fs, mesh::RNG &trng) {
    ...
    if (_fs->exists("/node_prefs")) {
      ...
      if (file) {
        ...
        file.read((uint8_t *)&_prefs.tx_power_dbm,
                  sizeof(_prefs.tx_power_dbm));                     // 68
        file.read((uint8_t *)_prefs.unused, sizeof(_prefs.unused)); // 69
        file.read((uint8_t *)&_prefs.rx_delay_base,
                  sizeof(_prefs.rx_delay_base));                       // 72
        file.read(pad, 4);                                             // 76
        ...
        _prefs.tx_power_dbm =
            constrain(_prefs.tx_power_dbm, 1, MAX_LORA_TX_POWER);
  ...
  void savePrefs() {
    ...
    if (file) {
      ...
      file.write((uint8_t *)&_prefs.tx_power_dbm,
                 sizeof(_prefs.tx_power_dbm));                     // 68
      file.write((uint8_t *)_prefs.unused, sizeof(_prefs.unused)); // 69
      file.write((uint8_t *)&_prefs.rx_delay_base,
                 sizeof(_prefs.rx_delay_base));                       // 72
      file.write(pad, 4);                                             // 76
  ...
  void handleCmdFrame(size_t len) {
    if (cmd_frame[0] == CMD_DEVICE_QEURY &&
        len >= 2) { // sent when app establishes connection
      app_target_ver =
          cmd_frame[1]; // which version of protocol does app understand
      ...
      i += 12;
      StrHelper::strzcpy((char *)&out_frame[i], board.getManufacturerName(),
                         40);
      i += 40;
      ...
      _serial->writeFrame(out_frame, i);
               ...
               len >= 8) { // sent when app establishes connection, respond with
                           // node ID
      ...
      out_frame[i++] = ADV_TYPE_CHAT; // what this node Advert identifies as
                                      // (maybe node's pronouns too?? :-)
      ...
      if (recipient && attempt < 4 &&
          (txt_type == TXT_TYPE_PLAIN || txt_type == TXT_TYPE_CLI_DATA)) {
        ...
        if (txt_type == TXT_TYPE_CLI_DATA) {
          ...
          result = sendMessage(*recipient, msg_timestamp, attempt, text,
                               expected_ack_crc, est_timeout);
        }
        ...
        writeErrFrame(); // unknown recipient, or unsuported TXT_TYPE_*
               ...
               CMD_SEND_CHANNEL_TXT_MSG) { // send GroupChannel msg
      int i = 1;
      ...
      if (txt_type == TXT_TYPE_PLAIN &&
          sendGroupMessage(msg_timestamp, *_public, _prefs.node_name, text,
                           len - i)) { // hard-coded to 'public' channel for now
        writeOKFrame();
      } else {
        writeErrFrame();
      }
      ...
      if (lat <= 90 * 1E6 && lat >= -90 * 1E6 && lon <= 180 * 1E6 &&
          lon >= -180 * 1E6) {
      ...
      if (pkt) {
        if (len >= 2 &&
            cmd_frame[1] == 1) { // optional param (1 = flood, 0 = zero hop)
          sendFlood(pkt);
        } else {
          sendZeroHop(pkt);
        }
      ...
      if (recipient) {
        ...
        // recipient->lastmod = ??   shouldn't be needed, app already has this
        // version of contact
        saveContacts();
        writeOKFrame();
      } else {
      ...
      if (len < 1 + PUB_KEY_SIZE) {
        // export SELF
        auto pkt = createSelfAdvert(_prefs.node_name, _prefs.node_lat,
                                    _prefs.node_lon);
        if (pkt) {
        ...
        uint8_t out_len;
        if (recipient &&
            (out_len = exportContact(*recipient, &out_frame[1])) > 0) {
          out_frame[0] = RESP_CODE_EXPORT_CONTACT;
          _serial->writeFrame(out_frame, out_len + 1);
        } else {
          writeErrFrame(); // not found
      ...
      if (freq >= 300000 && freq <= 2500000 && sf >= 7 && sf <= 12 && cr >= 5 &&
          cr <= 8 && bw >= 7000 && bw <= 500000) {
        _prefs.sf = sf;
        _prefs.cr = cr;
        ...
        _phy->setCodingRate(_prefs.cr);
        MESH_DEBUG_PRINTLN(
            "OK: CMD_SET_RADIO_PARAMS: f=%d, bw=%d, sf=%d, cr=%d", freq, bw,
            (uint32_t)sf, (uint32_t)cr);

        writeOKFrame();
      } else {
        MESH_DEBUG_PRINTLN(
            "Error: CMD_SET_RADIO_PARAMS: f=%d, bw=%d, sf=%d, cr=%d", freq, bw,
            (uint32_t)sf, (uint32_t)cr);
        writeErrFrame();
      }
      ...
      if (recipient) {
        ...
        if (result == MSG_SEND_FAILED) {
          ...
          pending_status = 0;
          memcpy(&pending_login, recipient->id.pub_key,
                 4); // match this to onContactResponse()
          out_frame[0] = RESP_CODE_SENT;
          out_frame[1] = (result == MSG_SEND_SENT_FLOOD) ? 1 : 0;
      ...
      if (recipient) {
        ...
        if (result == MSG_SEND_FAILED) {
          ...
          pending_login = 0;
          memcpy(&pending_status, recipient->id.pub_key,
                 4); // match this to onContactResponse()
          out_frame[0] = RESP_CODE_SENT;
          out_frame[1] = (result == MSG_SEND_SENT_FLOOD) ? 1 : 0;
  ...
  void loop() {
    ...
    if (len > 0) {
      handleCmdFrame(len);
               ...
               && !_serial->isWriteBusy() // don't spam the Serial Interface too
                                          // quickly!
    ) {
      ContactInfo contact;
      if (_iter.hasNext(this, contact)) {
        if (contact.lastmod > _iter_filter_since) { // apply the 'since' filter
          ...
          if (contact.lastmod > _most_recent_lastmod) {
            _most_recent_lastmod =
                contact.lastmod; // save for the RESP_CODE_END_OF_CONTACTS frame
          }
        }
      } else { // EOF
        out_frame[0] = RESP_CODE_END_OF_CONTACTS;
        memcpy(&out_frame[1], &_most_recent_lastmod,
               4); // include the most recent lastmod, so app can update their
                   // 'since'
...
#else
RADIO_CLASS radio =
    new Module(P_LORA_NSS, P_LORA_DIO_1, P_LORA_RESET, P_LORA_BUSY);
#endif
StdRNG fast_rng;
SimpleMeshTables tables;
MyMesh the_mesh(radio, *new WRAPPER_CLASS(radio, board), fast_rng,
                *new VolatileRTCClock(), tables);

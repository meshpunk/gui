...
class MyMesh : public mesh::Mesh, public CommonCLICallbacks {
  ...
  ClientInfo *putClient(const mesh::Identity &id) {
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
      ...
      if (id.matches(known_clients[i].id))
        return &known_clients[i]; // already known
    }
  ...
  int handleRequest(ClientInfo *sender, uint8_t *payload, size_t payload_len) {
    ...
    memcpy(reply_data, &now,
           4); // response packets always prefixed with timestamp
...
protected:
  ...
  void logRx(mesh::Packet *pkt, int len, float score) override {
    if (_logging) {
      ...
      if (f) {
        ...
        f.printf(": RX, len=%d (type=%d, route=%s, payload_len=%d) SNR=%d "
                 "RSSI=%d score=%d",
        ...
        if (pkt->getPayloadType() == PAYLOAD_TYPE_PATH ||
          ...
          f.printf(" [%02X -> %02X]\n", (uint32_t)pkt->payload[1],
                   (uint32_t)pkt->payload[0]);
        } else {
          f.printf("\n");
        }
        f.close();
  ...
  void logTx(mesh::Packet *pkt, int len) override {
    if (_logging) {
      ...
      if (f) {
        f.print(getLogDateTime());
        f.printf(": TX, len=%d (type=%d, route=%s, payload_len=%d)", len,
  ...
  void onAnonDataRecv(mesh::Packet *packet, uint8_t type,
                      const mesh::Identity &sender, uint8_t *data,
                      size_t len) override {
    if (type ==
        PAYLOAD_TYPE_ANON_REQ) { // received an initial request by a possible
                                 // admin client (unknown at this stage)
      uint32_t timestamp;
      memcpy(&timestamp, data, 4);

      bool is_admin;
      data[len] = 0; // ensure null terminator
      if (strcmp((char *)&data[4], _prefs.password) ==
          0) { // check for valid password
        is_admin = true;
      } else if (strcmp((char *)&data[4], _prefs.guest_password) ==
                 0) { // check guest password
        is_admin = false;
      } else {
#if MESH_DEBUG
      ...
      auto client =
          putClient(sender); // add to known clients (if not already known)
      ...
      getRNG()->random(&reply_data[8],
                       4); // random blob to help packet-hash uniqueness
#endif
      ...
      if (packet->isRouteFlood()) {
        ...
        mesh::Packet *path = createPathReturn(
            sender, client->secret, packet->path, packet->path_len,
            PAYLOAD_TYPE_RESPONSE, reply_data, 12);
        if (path)
          sendFlood(path);
      } else {
        mesh::Packet *reply = createDatagram(PAYLOAD_TYPE_RESPONSE, sender,
                                             client->secret, reply_data, 12);
        if (reply) {
          if (client->out_path_len >=
              0) { // we have an out_path, so send DIRECT
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
      if (known_clients[i].id.isHashMatch(hash)) {
        matching_peer_indexes[n++] =
            i; // store the INDEXES of matching contacts (for subsequent 'peer'
               // methods)
      }
    }
    return n;
  ...
  void onPeerDataRecv(mesh::Packet *packet, uint8_t type, int sender_idx,
    ...
    if (i < 0 || i >= MAX_CLIENTS) { // get from our known_clients table (sender
                                     // SHOULD already be known in this context)
    ...
    if (type == PAYLOAD_TYPE_REQ) { // request (from a Known admin client!)
      ...
      if (timestamp > client->last_timestamp) { // prevent replay attacks
        ...
        if (packet->isRouteFlood()) {
          // let this sender know path TO here, so they can use sendDirect(),
          // and ALSO encode the response
          mesh::Packet *path = createPathReturn(
        ...
        MESH_DEBUG_PRINTLN("onPeerDataRecv: possible replay attack detected");
               ...
               client->is_admin) { // a CLI command
      uint32_t sender_timestamp;
      memcpy(&sender_timestamp, data,
             4); // timestamp (by sender's RTC clock - which could be wrong)
      uint flags = (data[4] >> 2); // message attempt number, and other flags

      if (!(flags == TXT_TYPE_PLAIN || flags == TXT_TYPE_CLI_DATA)) {
        MESH_DEBUG_PRINTLN(
            "onPeerDataRecv: unsupported text type received: flags=%02x",
            (uint32_t)flags);
      } else if (sender_timestamp >=
                 client->last_timestamp) { // prevent replay attacks
        ...
        if (flags == TXT_TYPE_PLAIN) { // for legacy CLI, send Acks
          uint32_t
              ack_hash; // calc truncated hash of the message timestamp + text +
                        // sender pub_key, to prove to sender that we got it
        ...
        if (is_retry) {
          ...
          _cli.handleCommand(sender_timestamp, (const char *)&data[5],
                             (char *)&temp[5]);
        }
        ...
        if (text_len > 0) {
          ...
          memcpy(temp, &timestamp,
                 4); // mostly an extra blob to help make packet_hash unique
          temp[4] =
              (TXT_TYPE_CLI_DATA << 2); // NOTE: legacy was: TXT_TYPE_PLAIN
  ...
  bool onPeerPathRecv(mesh::Packet *packet, int sender_idx,
    ...
    if (i >= 0 && i < MAX_CLIENTS) { // get from our known_clients table (sender
      ...
      auto client = &known_clients[i];
      memcpy(client->out_path, path,
             client->out_path_len =
                 path_len); // store a copy of path, for sendDirect()
    } else {
...
public:
  MyMesh(RADIO_CLASS &phy, mesh::MainBoard &board, RadioLibWrapper &radio,
         mesh::MillisecondClock &ms, mesh::RNG &rng, mesh::RTCClock &rtc,
         SimpleMeshTables &tables)
      : mesh::Mesh(radio, ms, rng, rtc, *new StaticPoolPacketManager(32),
                   tables),
    ...
    StrHelper::strncpy(_prefs.password, ADMIN_PASSWORD,
...
MyMesh the_mesh(radio, board, *new WRAPPER_CLASS(radio, board),
                *new ArduinoMillis(), fast_rng, rtc_clock, tables);
